...
async function applyProposalToWorkspace(options: {
  ...
  if (proposal.contract) {
    ...
    const contractData = {
      organization_id: ctx.organizationId,
      client_id: clientId,
      client_references: normalizeOptionalString(contractPayload.client_references ?? ''),
      contract_type: contractPayload.contract_type?.trim() || 'standard',
      duration_terms: normalizeOptionalString(contractPayload.duration_terms ?? ''),
      status: 'active',
      start_date: normalizeDateForDb(contractPayload.start_date),
      issue_date: normalizeDateForDb(contractPayload.issue_date),
      renewal_date: normalizeDateForDb(contractPayload.renewal_date),
      end_date: normalizeDateForDb(contractPayload.end_date),
      exercised_activity: normalizeOptionalString(contractPayload.exercised_activity ?? ''),
      protocol_code: normalizeOptionalString(contractPayload.protocol_code ?? ''),
      ...
      supervisor_name: normalizeOptionalString(contractPayload.supervisor_name ?? ''),
      internal_owner: normalizeOptionalString(contractPayload.internal_owner ?? ''),
      validity_terms: normalizeOptionalString(contractPayload.validity_terms ?? ''),
